...
(async()=>{try{
 ...
 const student=cookies['student.a'],row={primary:true,univ:'동일대',admission:'종합',major:'수학',interviewType:'생기부 기반',firstDate:'2026-10-01',interviewDate:'2026-11-01',finalDate:'2026-12-01',firstResult:'',finalResult:'',direct:false},rows=[row,{...row,primary:false,admission:'교과',direct:true},...Array.from({length:4},()=>({}))];
 assert.equal((await req('/api/applications',student,'PUT',{rows})).status,200);
 assert.equal((await req('/api/state',student)).data.applications[0].rows[0].primary,true);
 ...
 for(const invalid of [rows.map(r=>({...r,primary:false})),rows.map((r,i)=>i===1?{...r,primary:true}:r),rows.map((r,i)=>i===2?{primary:true}:r),rows.slice(0,5),[...rows,{}],rows.map((r,i)=>i?r:{...r,interviewType:'임의'}),rows.map((r,i)=>i?r:{...r,interviewDate:'2026-02-30'}),rows.map((r,i)=>i?r:{...r,finalDate:'2026-09-01'})])assert.equal((await req('/api/applications',student,'PUT',{rows:invalid})).status,400);
 ...
 rows[0].primary=false;rows[1].primary=true;assert.equal((await req('/api/applications',student,'PUT',{rows})).status,200);await app.close();await start();assert.equal((await req('/api/state',student)).data.applications[0].rows[1].primary,true);
 assert.equal((await req('/api/applications',student,'PUT',{rows:Array.from({length:6},()=>({}))})).status,200);assert.equal((await req('/api/state',student)).data.events.length,0);
