...
function createApp({dataDir=process.env.DATA_DIR||path.join(__dirname,'data'),secureCookies=false}={}){
 ...
 db.exec('CREATE TABLE IF NOT EXISTS upload_batches(id TEXT PRIMARY KEY,owner TEXT NOT NULL,path TEXT NOT NULL,method TEXT NOT NULL,parts INTEGER NOT NULL,received INTEGER NOT NULL,expires INTEGER NOT NULL)');
 let uploadRoute,resolveUpload,submissionRoute,managementState,managementRoute,deleteAccountData,decorateEvents,saveAdmissionResult,saveApplicationRows,validateApplications,applicationEvents,lessonVisible,lessonFields,saveStudentAvailability,validateAvailability,resourceRoute;
 ...
 const ready=(async()=>{({uploadRoute,resolveUpload}=await import('./transfers.mjs'));({submissionRoute}=await import('./submissions.mjs'));({validateApplications,applicationEvents}=await import('./applications.mjs'));({lessonVisible,lessonFields,saveStudentAvailability,validateAvailability,decorateEvents}=await import('./school.mjs'));({resourceRoute}=await import('./resources.mjs'));({saveAdmissionResult,saveApplicationRows}=await import('./results.mjs'));({managementState,managementRoute,deleteAccountData}=await import('./management.mjs'));if(!getUser('admin'))db.prepare('INSERT INTO users(id,number,name,role,class,password_hash) VALUES(?,?,?,?,?,?)').run('admin','','관리자','관리자','',await hashPassword(process.env.ADMIN_INITIAL_PASSWORD||'admin123'));})();
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  try{
   ...
   if(route.startsWith('/api/')){
    ...
    if(route.startsWith('/api/uploads')){const result=await uploadRoute({url:'http://localhost'+req.url,method:req.method},u,storage.DB,storage.FILES,()=>body(req));res.writeHead(result.status,Object.fromEntries(result.headers));return res.end(Buffer.from(await result.arrayBuffer()));}
    if(route.startsWith('/api/submissions')){const result=await submissionRoute({url:'http://localhost'+req.url,method:req.method},u,storage.DB,storage.FILES,async()=>resolveUpload(await body(req),u,storage.DB,storage.FILES,route,req.method));res.writeHead(result.status,Object.fromEntries(result.headers));return res.end(Buffer.from(await result.arrayBuffer()));}
    if(route.startsWith('/api/resources')){const request={url:'http://localhost'+req.url,method:req.method};const result=await resourceRoute(request,u,storage.DB,storage.FILES,async()=>resolveUpload(await body(req),u,storage.DB,storage.FILES,route,req.method));res.writeHead(result.status,Object.fromEntries(result.headers));return res.end(Buffer.from(await result.arrayBuffer()));}
