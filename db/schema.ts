...
export const uploadBatches=sqliteTable('upload_batches',{id:text('id').primaryKey(),owner:text('owner').notNull(),path:text('path').notNull(),method:text('method').notNull(),parts:integer('parts').notNull(),received:integer('received').notNull(),expires:integer('expires').notNull()});
