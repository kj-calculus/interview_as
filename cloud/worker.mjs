...
async function readBody(request){if(request.headers.get('content-type')?.split(';')[0]!=='application/json'||request.headers.get('x-interview-request')!=='1')throw fail(415,'올바른 요청 형식이 아닙니다.');if(Number(request.headers.get('content-length'))>1048576)throw fail(413,'요청이 너무 큽니다.');const reader=request.body?.getReader();let size=0;const chunks=[];if(reader)while(true){const {done,value}=await reader.read();if(done)break;size+=value.length;if(size>1048576){await reader.cancel();throw fail(413,'요청이 너무 큽니다.');}chunks.push(value);}const bytes=new Uint8Array(size);let offset=0;for(const c of chunks){bytes.set(c,offset);offset+=c.length;}try{const b=JSON.parse(new TextDecoder().decode(bytes));if(!b||typeof b!=='object'||Array.isArray(b))throw Error();return b;}catch{throw fail(400,'입력 내용을 확인해주세요.');}}
...
export async function handle(request,env,assets={}) {
 ...
 try {
  ...
  const canEvent=async(u,e)=>e.type==='lesson'?(u.role==='관리자'||u.class===e.class):canStudent(u,await getUser(e.student));
  ...
  async function rowsFor(u,rows){manager(u);if(!Array.isArray(rows)||!rows.length||rows.length>500)throw fail(400,'계정은 한 번에 1~500명까지 등록할 수 있습니다.');const existing=await users(),ids=new Set(existing.map(a=>a.id.toLowerCase())),numbers=new Set(existing.filter(a=>a.role==='학생').map(a=>a.number));return rows.map((r,i)=>{const a={row:Number.isInteger(r?.row)?r.row:i+2};let error='';try{for(const [k,max] of Object.entries({id:40,name:60,number:30,role:10,class:80}))a[k]=text(r?.[k]??'',max,k!=='number');if(!['교사','학생'].includes(a.role))error='권한은 교사 또는 학생만 가능합니다.';else if(u.role==='교사'&&(a.role!=='학생'||a.class!==u.class))error='본인 배정반의 학생만 등록할 수 있습니다.';else if(a.role==='학생'&&!a.number)error='학생 학번은 필수입니다.';else if(!/^[a-zA-Z0-9._-]{3,40}$/.test(a.id))error='아이디는 영문·숫자·._- 3~40자입니다.';else if(ids.has(a.id.toLowerCase()))error='중복된 아이디입니다.';else if(a.role==='학생'&&numbers.has(a.number))error='중복된 학생 학번입니다.';ids.add(a.id.toLowerCase());if(a.role==='학생')numbers.add(a.number);}catch(e){error=e.message;}return {...a,error};});}
  async function eventFor(u,b){manager(u);const type=text(b.type,20),date=text(b.date,10),time=text(b.time,5);if(!['lesson','first','interview','final'].includes(type))throw fail(400,'일정 종류를 확인해주세요.');if(!/^\d{4}-\d{2}-\d{2}$/.test(date)||!Number.isFinite(Date.parse(date))||new Date(date).toISOString().slice(0,10)!==date||date<'2000-01-01'||date>'2100-12-31')throw fail(400,'올바른 날짜를 입력해주세요.');if(!/^([01]\d|2[0-3]):[0-5]\d$/.test(time))throw fail(400,'올바른 시간을 입력해주세요.');const e={type,date,time,student:null,class:'',univ:'',major:'',title:'',task:''};if(type==='lesson'){e.class=text(b.class,80);e.title=text(b.title,80);e.task=text(b.task??'',5000,false);if(u.role==='교사'&&e.class!==u.class)throw fail(403,'본인 배정반에만 수업을 등록할 수 있습니다.');if(!(await users()).some(a=>a.class===e.class))throw fail(400,'계정을 먼저 등록해 배정반을 만들어주세요.');}else{const s=await getUser(text(b.student,40));if(!canStudent(u,s))throw fail(403,'해당 학생의 일정을 관리할 수 없습니다.');e.student=s.id;e.class=s.class;e.univ=text(b.univ,100);e.major=text(b.major??'',100,false);}return e;}
