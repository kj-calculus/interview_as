...
(async()=>{
 ...
 run("showImportant=true;showUniversity=false;events=[{id:'school',type:'important',title:'수능',date:'2026-11-19',time:''}];");assert.equal(run('filteredCalendarEvents(events).length'),1);assert.ok(run('visibleEvents().some(e=>e.type===\'important\')'));run('showImportant=false;');assert.equal(run('filteredCalendarEvents(events).length'),0);
 run("role='교사';identity='teacher.a';progressClass='all';events=[{id:'group',type:'lesson',class:'면접 A반',lesson_kind:'1차-OT',title:'1차-OT',date:'2026-11-19',time:'7교시',target_ids:'null'}];progress=[{event_id:'group',student:'student.a'}];");assert.ok(run('afterschoolView()').includes('완료 1/1회'));assert.ok(run('afterschoolView()').includes('미등록'));
