...
export async function handle(request,env,assets={}) {
 ...
 try {
  ...
  if(route==='/api/state'&&method==='GET') {const list=await users(),accounts=list.filter(a=>u.role==='관리자'||a.id===u.id||canStudent(u,a));const events=(await all('SELECT * FROM events ORDER BY date,time')).filter(e=>e.type==='lesson'?(u.role==='관리자'||classesOf(u).includes(e.class)):canStudent(u,list.find(a=>a.id===e.student)));return json(200,{user:publicUser(u),accounts:accounts.map(publicUser),events});}
