...
function adapters(db,dataDir){
 ...
 const FILES={async put(id,bytes){await fs.promises.writeFile(file(id),bytes);},async get(id,options){try{const bytes=await fs.promises.readFile(file(id));return {body:options?.range?bytes.subarray(options.range.offset,options.range.offset+options.range.length):bytes};}catch(e){if(e.code==='ENOENT')return null;throw e;}},async delete(id){await fs.promises.rm(file(id),{force:true});}};
 return {DB,FILES};
}
