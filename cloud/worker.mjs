import {lessonVisible,lessonFields} from '../school.mjs';
...
async function readBody(request){const limit=new URL(request.url).pathname.startsWith('/api/resources')?3*1048576:1048576;if(request.headers.get('content-type')?.split(';')[0]!=='application/json'||request.headers.get('x-interview-request')!=='1')throw fail(415,'올바른 요청 형식이 아닙니다.');if(Number(request.headers.get('content-length'))>limit)throw fail(413,'요청이 너무 큽니다.');const reader=request.body?.getReader();let size=0;const chunks=[];if(reader)while(true){const {done,value}=await reader.read();if(done)break;size+=value.length;if(size>limit){await reader.cancel();throw fail(413,'요청이 너무 큽니다.');}chunks.push(value);}const bytes=new Uint8Array(size);let offset=0;for(const c of chunks){bytes.set(c,offset);offset+=c.length;}try{const b=JSON.parse(new TextDecoder().decode(bytes));if(!b||typeof b!=='object'||Array.isArray(b))throw Error();return b;}catch{throw fail(400,'입력 내용을 확인해주세요.');}}
...
export async function handle(request,env,assets={}) {
 ...
 try {
  ...
  if(route==='/api/state'&&method==='GET') {const list=await users(),accounts=list.filter(a=>u.role==='관리자'||a.id===u.id||canStudent(u,a));const events=(await all('SELECT * FROM events ORDER BY date,time')).filter(e=>e.type==='lesson'?lessonVisible(u,e):canStudent(u,list.find(a=>a.id===e.student)));const applications=(await all('SELECT * FROM applications')).filter(a=>canStudent(u,list.find(s=>s.id===a.student))).map(a=>({...a,rows:JSON.parse(a.rows)}));return json(200,{user:publicUser(u),accounts:accounts.map(publicUser),applications,events:[...events,...applicationEvents(applications,accounts)]});}
  ...
  if(route==='/api/events'&&method==='POST'){const e=await eventFor(u,await readBody(request)),id=crypto.randomUUID();await run('INSERT INTO events(id,type,student,class,univ,major,date,time,title,task,lesson_kind,teacher_name,target_ids) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',id,e.type,e.student,e.class,e.univ,e.major,e.date,e.time,e.title,e.task,e.lesson_kind||'',e.teacher_name||'',e.target_ids||'null');return json(201,{id});}
  const match=route.match(/^\/api\/events\/([a-zA-Z0-9-]+)$/);
  if(match&&['PUT','DELETE'].includes(method)){manager(u);const b=await readBody(request),old=await first('SELECT * FROM events WHERE id=?',match[1]);if(!old||!await canEvent(u,old))throw fail(404,'일정을 찾을 수 없습니다.');if(method==='DELETE')await run('DELETE FROM events WHERE id=?',old.id);else{const e=await eventFor(u,b,old);await run('UPDATE events SET type=?,student=?,class=?,univ=?,major=?,date=?,time=?,title=?,task=?,lesson_kind=?,teacher_name=?,target_ids=? WHERE id=?',e.type,e.student,e.class,e.univ,e.major,e.date,e.time,e.title,e.task,e.lesson_kind||'',e.teacher_name||'',e.target_ids||'null',old.id);}return json(200,{ok:true});}
