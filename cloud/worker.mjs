...
export async function handle(request,env,assets={}) {
 ...
 try {
  ...
  const canEvent=async(u,e)=>e.type==='lesson'?(u.role==='관리자'||classesOf(u).includes(e.class)):canStudent(u,await getUser(e.student));
  ...
  if(route==='/api/state'&&method==='GET') {const list=await users(),accounts=list.filter(a=>u.role==='관리자'||a.id===u.id||canStudent(u,a));const events=(await all('SELECT * FROM events ORDER BY date,time')).filter(e=>e.type==='lesson'?(u.role==='관리자'||classesOf(u).includes(e.class)):canStudent(u,list.find(a=>a.id===e.student)));const applications=(await all('SELECT * FROM applications')).filter(a=>canStudent(u,list.find(s=>s.id===a.student))).map(a=>({...a,rows:JSON.parse(a.rows)}));return json(200,{user:publicUser(u),accounts:accounts.map(publicUser),applications,events:[...events,...applicationEvents(applications,accounts)]});}
  if(route==='/api/applications'&&method==='PUT'){if(u.role!=='학생')throw fail(403,'학생 본인만 지원 정보를 입력할 수 있습니다.');const b=await readBody(request);if(b.student&&b.student!==u.id)throw fail(403,'본인 지원 정보만 입력할 수 있습니다.');const rows=validateApplications(b.rows);await run('INSERT INTO applications(student,rows) VALUES(?,?) ON CONFLICT(student) DO UPDATE SET rows=excluded.rows',u.id,JSON.stringify(rows));return json(200,{ok:true});}
  ...
  if(route==='/api/accounts/delete'&&method==='POST'){
   if(u.role!=='관리자')throw fail(403,'관리자만 계정을 삭제할 수 있습니다.');const b=await readBody(request),target=await getUser(text(b.id,40));if(!target)throw fail(404,'계정을 찾을 수 없습니다.');if(target.role==='관리자')throw fail(403,'관리자 계정은 삭제할 수 없습니다.');await db.batch([statement('DELETE FROM sessions WHERE user_id=?',[target.id]),statement('DELETE FROM events WHERE student=?',[target.id]),statement('DELETE FROM users WHERE id=?',[target.id])]);return json(200,{ok:true});
  }
