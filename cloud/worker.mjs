...
import {lessonVisible,lessonFields,saveStudentAvailability,validateAvailability,decorateEvents} from '../school.mjs';
...
export async function handle(request,env,assets={}) {
 ...
 try {
  ...
  if(route==='/api/applications'&&method==='PUT'){const b=await readBody(request),student=b.student||u.id;if(!canStudent(u,await getUser(student)))throw fail(403,'해당 학생의 지원 정보를 수정할 수 없습니다.');const rows=validateApplications(b.rows);await saveApplicationRows(db,student,rows,b.expectedRows);return json(200,{ok:true});}
