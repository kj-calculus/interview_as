...
export async function resourceRoute(request,u,db,bucket,readBody){
 ...
 if(route==='/api/resources'&&method==='GET'){
  const posts=await all('SELECT * FROM resource_posts ORDER BY created_at DESC LIMIT 200');
  const files=await all('SELECT id,post_id,name,size FROM resource_files WHERE post_id IN (SELECT id FROM resource_posts ORDER BY created_at DESC LIMIT 200)');
  return json(200,{posts:posts.map(p=>({...p,files:files.filter(f=>f.post_id===p.id),editable:p.author_id===u.id||u.role==='관리자'}))});
 }
 const fileMatch=route.match(/^\/api\/resources\/files\/([a-f0-9-]+)$/);
 if(fileMatch&&method==='GET'){
  const f=await first('SELECT * FROM resource_files WHERE id=?',fileMatch[1]);if(!f)throw resourceError(404,'첨부파일을 찾을 수 없습니다.');
 ...
 if(route==='/api/resources'&&method==='POST'){
  const b=await readBody(request),title=resourceText(b.title,120),content=resourceText(b.content??'',20000,false),files=b.files??[];
 ...
 if(match&&['PUT','DELETE'].includes(method)){
  ...
  if(method==='PUT'){const title=resourceText(b.title,120),content=resourceText(b.content??'',20000,false);if(!content&&!(await first('SELECT id FROM resource_files WHERE post_id=?',p.id)))throw resourceError(400,'내용을 입력해주세요.');await stmt('UPDATE resource_posts SET title=?,content=? WHERE id=?',title,content,p.id).run();}
