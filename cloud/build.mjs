...
fs.writeFileSync('dist/server/index.js',['applications.mjs','school.mjs','resources.mjs','results.mjs','management.mjs','submissions.mjs'].map(p=>fs.readFileSync(p,'utf8').replace(/^import .* from .*;\r?\n/gm,'')).join('\n')+'\n'+fs.readFileSync('cloud/worker.mjs','utf8').replace(/^import .* from '\.\.\/.*';\r?\n/gm,'')+'\nconst assets='+JSON.stringify(assets)+';\nexport default {fetch(request,env){return handle(request,env,assets);}};\n');
