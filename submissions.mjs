...
export async function submissionRoute(request,u,db,bucket,readBody){
 ...
 if(path==='/api/submissions'&&method==='GET'){
  ...
  return json(200,{lessons:lessons.map(e=>({...e,editable:u.role==='관리자'||u.role==='교사'&&e.author_id===u.id,files:assignmentFiles.filter(f=>f.assignment_id===e.id),target_ids:u.role==='학생'?'null':e.target_ids})),students,submissions:submissions.map(s=>({...s,files:files.filter(f=>f.submission_id===s.id)}))});
 }
 ...
 if(['/api/submissions','/api/submissions/assignments'].includes(path)&&method==='POST'||editMatch&&method==='PUT'){
  ...
  if(editMatch&&(!old||!lessonVisible(u,old)||u.role!=='관리자'&&old.author_id!==u.id))throw submissionError(403,'작성 교사 또는 관리자만 과제를 수정할 수 있습니다.');
  ...
  let total=0;const files=b.files.map(f=>{if(typeof f?.name!=='string'||!f.name.trim()||f.name.length>180||typeof f.data!=='string'||f.data.length>14000000||(f.data.length%4!==0||!/^[A-Za-z0-9+/]*={0,2}$/.test(f.data)))throw submissionError(400,'첨부파일을 확인해주세요.');const bytes=decodeAttachment(f.data);total+=bytes.length;if(!bytes.length||total>10*1024*1024)throw submissionError(413,'파일은 합계 10MB 이하로 제출해주세요.');return {id:crypto.randomUUID(),name:f.name.trim().replace(/[\r\n\\/]/g,'_'),bytes};});
  ...
  const oldFiles=old?await all('SELECT * FROM assignment_files WHERE assignment_id=?',old.id):[];
  const keepIds=b.keep_files??oldFiles.map(f=>f.id);if(!Array.isArray(keepIds)||keepIds.some(id=>!oldFiles.some(f=>f.id===id)))throw submissionError(400,'유지할 첨부파일을 확인해주세요.');const kept=oldFiles.filter(f=>keepIds.includes(f.id)),removed=oldFiles.filter(f=>!keepIds.includes(f.id));
  if(kept.length+files.length>3||kept.reduce((n,f)=>n+f.size,0)+total>10*1024*1024)throw submissionError(413,'기존 파일과 새 파일은 합계 3개·10MB 이하여야 합니다.');
  const content=b.content.trim();if(!content&&!files.length&&!kept.length)throw submissionError(400,'제출 글 또는 파일을 입력해주세요.');if(files.length&&!bucket)throw submissionError(503,'파일 저장소에 연결하지 못했습니다.');
  const id=old?.id||crypto.randomUUID(),uploaded=[];
  try{for(const f of files){await bucket.put(f.id,f.bytes);uploaded.push(f.id);}await db.batch(creating?[...(old?[stmt('UPDATE assignments SET title=?,content=?,class=?,target_ids=? WHERE id=?',b.title.trim(),content,b.class,target_ids,id),...removed.map(f=>stmt('DELETE FROM assignment_files WHERE id=?',f.id))]:[stmt('INSERT INTO assignments(id,title,content,class,target_ids,author_id,author_name,created_at) VALUES(?,?,?,?,?,?,?,?)',id,b.title.trim(),content,b.class,target_ids,u.id,u.name,new Date().toISOString())]),...files.map(f=>stmt('INSERT INTO assignment_files(id,assignment_id,name,size) VALUES(?,?,?,?)',f.id,id,f.name,f.bytes.length))]:[stmt('INSERT INTO assignment_submissions(id,event_id,student,content,created_at) VALUES(?,?,?,?,?)',id,b.event,u.id,content,new Date().toISOString()),...files.map(f=>stmt('INSERT INTO submission_attachments(id,submission_id,name,size) VALUES(?,?,?,?)',f.id,id,f.name,f.bytes.length))]);}catch(e){for(const id of uploaded)try{await bucket.delete(id);}catch{}throw e;}
  for(const f of removed)try{await bucket?.delete(f.id);}catch{}
  return json(old?200:201,{id});
 }
