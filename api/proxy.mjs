...
const allowed=/^(?:login|logout|me|password|state|important-events(?:\/[a-f0-9-]+)?|lesson-progress|submissions(?:\/files\/[a-f0-9-]+)?|applications|results|availability|resources(?:\/(?:files\/)?[a-f0-9-]+)?|accounts(?:\/(?:validate|reset-password|classes|delete))?|events(?:\/[a-zA-Z0-9-]+)?)$/;
export async function handle(request,send=fetch){
 ...
 try{
  ...
  if(request.method==='GET'&&/^(?:resources|submissions)\/files\/[a-f0-9-]+$/.test(path)&&response.ok){return new Response(response.body,{status:200,headers:{...headers,'Content-Type':'application/octet-stream','Content-Disposition':response.headers.get('content-disposition')||'attachment','Content-Security-Policy':"default-src 'none'; sandbox"}});}
