...
const allowed=/^(?:login|logout|me|password|state|applications|accounts(?:\/(?:validate|reset-password|classes|delete))?|events(?:\/[a-zA-Z0-9-]+)?)$/;
export async function handle(request,send=fetch){
 ...
 if(request.method!=='GET'){
  ...
  if(reader)while(true){const chunk=await reader.read();if(chunk.done)break;size+=chunk.value.length;if(size>1048576){await reader.cancel();return error(413,'요청이 너무 큽니다.');}chunks.push(chunk.value);}
